    ...
    def fit(self, train_data, valid_data=None, test_data=None, saved=False, verbose=True):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if update_flag:
                    torch.save(self.model.state_dict(), self.config['dataset'] + "_best_model.pt")
...
class Trainer_KL(AbstractTrainer):
    r"""The basic Trainer for basic training and evaluation strategies in recommender systems. This class defines common
    functions for training and evaluation processes of most recommender system models, including fit(), evaluate(),
   and some other features helpful for model training and evaluation.
    ...
    def __init__(self, config, model):
        super(Trainer_KL, self).__init__(config, model)
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if verbose:
                    ...
                    self.logger.info('test result: \n' + dict2str(test_result))
                if update_flag:
                    # # torch.save(self.model.state_dict(), self.config['dataset'] + "_.pt")
                    # item_embs,use_emb = self.model.predict_all()
                    # torch.save(item_embs, './item_emb/' + self.config['dataset'] + '_'+self.config['mod'] + "_item.pt")
                    # torch.save(use_emb, './item_emb/' + self.config['dataset'] + '_'+self.config['mod'] + "_user.pt")
                    update_output = '██ ' + self.config['model'] + '--Best validation results updated!!!'
                    if verbose:
                        self.logger.info(update_output)
                    self.best_valid_result = valid_result
